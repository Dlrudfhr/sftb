  ...
  const fetchRanking = async () => {
    try {
      ...
      const sortedData = response.data.sort((a, b) => {
        ...
        if (tierComparison !== 0) return tierComparison; // 티어가 다르면 티어로 정렬

        // 레벨 비교
        else if (a.userLevel !== b.userLevel) return b.userLevel - a.userLevel; // 레벨이 다르면 레벨로 정렬

        // 티어 경험치 비교
        else if (a.tierExperience !== b.tierExperience) return b.tierExperience - a.tierExperience; // 티어 경험치로 정렬

        // 레벨 경험치 비교
        else if (a.userLevelExperience !== b.userLevelExperience) return b.userLevelExperience - a.userLevelExperience; // 레벨 경험치로 정렬

        // 사용자 이름 비교 (가나다라 순으로 정렬)
        else
        return a.userName.localeCompare(b.userName); // 사용자 이름으로 오름차순 정렬
      });
